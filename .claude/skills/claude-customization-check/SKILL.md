...
- **CRLF のファイルは frontmatter 最終行が落ちる**（修正済み・再発注意）。JS の正規表現の `.` は `\r` を行終端として扱うため、`key: value$` が CRLF 行に一致しない。閉じ `---` の直前のキーだけが黙って欠落し、`tools` や `description` が「無い」と誤判定される。パーサは先に `\r\n` → `\n` へ正規化すること。Python の `pathlib.write_text` は Windows で CRLF を書くので、スクリプトで一括置換したあとに踏みやすい
