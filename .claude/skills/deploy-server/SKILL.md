---
name: deploy-server
description: Railway 本番サーバー（だいどこ側の apps/server）へのデプロイと疎通確認。環境変数の確認、railway up、デプロイ完了ポーリング、/health と AI エンドポイントのスモークテストまで。
---
...
> **サーバーコードはこのリポジトリに無い（#150・2026-09-08）。** AI 推論はだいどこの Railway を
> 共用しており（決定⑨）、正は `C:\Projects\daidoko` の `apps/server`。 <!-- daidoko-ref-ok -->
> **デプロイも疎通確認もだいどこのリポジトリ側で行う** — ここから `railway up` しても送るものが無い。
> この手順書は、さいえん手帳専用のサービスを立てる判断をしたときに使う骨組みとして残している。
