...
<!-- path-ref-ok: 停止中のスキル。下に出てくるスクリプト・アセットのパスは移植元の値で、さいえん手帳側は未作成。差し替えるときにこの行も消す -->

> **このファイルのパスは実在しない。** 手順書のパス実在チェック
> （`scripts/agent/validate-claude-customizations.mjs`）を上の宣言で丸ごと免除している。
> **さいえん手帳用に書き直すときは、まず宣言を消して落ちる箇所を潰すこと。**

collect（データはコード）→ record（screenrecord）→ edit（ffmpeg）→ review（フレーム確認）→ handoff（YouTube アップロードはユーザー作業）の一直線フロー。詳細は `docs/リリース手順.md`（未整備 — WBS 3.9 で作成） §3-4、ストア掲載全般は `update-store-listing` Skill。
