...
詳細は `docs/リリース手順.md`（未整備 — WBS 3.9 で作成） §3。プライバシーポリシーの公開反映は §4（公開 URL は gist — 更新時は同期コマンドを実行）。
...
- データセーフティフォームは API 非対応（Console UI / ブラウザ自動化）— 回答ガイドは `docs/リリース手順.md`（未整備 — WBS 3.9 で作成） §4
