...
栽培暦(50 品目・作物ごとに出典明記)・「今月の菜園仕事」・作物ガイド・「次の作業」・地域設定・
ホーム統合・**AI 相談(サーバー経由・だいどこ Railway 共用・決定⑨)**・
広告 3 形式(起動/リワード/バナー・実 ID 投入済み)。

**次は T1/T2(テスト整備)と v1.5(AI・統計)。** 4.19 第 1 段(作物マスター 50 品目・列の拡張)は 2026-09-05 に実装(#180)。 WBS 3.12a〜e は完了(残りは AdMob さいえん iOS の「要審査」だけで、クローラ待ち)。
