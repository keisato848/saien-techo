...
**ステージ:** 要件定義完了・**fork 未実施**(最初の実装タスクは WBS 1.1)

だいどこ(C:\Projects\daidoko)を fork して開発する。コンセプトは「私設・非 SNS・ローカルファースト」。

## 2. ドキュメントインデックス(実装前に必ず参照)

| ファイル                      | 内容                                                         |
| ----------------------------- | ------------------------------------------------------------ |
| `docs/要件定義.md`            | 決定事項ログ・機能要件(R01〜R31)・収益化・フェーズ           |
...
- フックは `node scripts/agent/*.mjs` を参照(配線済み)
- **だいどこ固有の参照が残っている**: `pnpm agent:*` スクリプト(package.json 未移植)、アプリ ID・ストア掲載・AdMob/RevenueCat の ID 類。WBS 1.1(fork)と WBS 3.7〜3.9 で さいえん手帳用に差し替えること
